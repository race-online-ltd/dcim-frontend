...
import { fetchUpsList, deleteUps } from "../../api/settings/upsApi";
...
const Ups = () => {
  ...
  const handleEdit = useCallback((id) => {
    navigate(`/admin/settings/ups-edit/${id}`);
  }, [navigate]);

  const handleDelete = useCallback((id) => {
    confirmAlert({
      title: 'Confirm to delete',
      message: 'Are you sure you want to delete this UPS?',
      buttons: [
        {
          label: 'Yes',
          onClick: async () => {
            try {
              const res = await deleteUps(id);
              successMessage(res);
              setUpsData(prev => prev.filter(item => item.id !== id));
            } catch (err) {
              errorMessage(err);
            }
          },
  ...
  }, [deleteUps, successMessage, errorMessage]);

  const handleExport = () => {
    console.log('Export UPS data');
  };
  ...
  return (
    <>
      ...
      <div className="ups-list-container">
        <header className="ups-header">
            ...
            <div className="d-flex gap-2">
                <Button
                    intent="primary"
                    leftIcon={Plus}
                    onClick={() => navigate('/admin/settings/ups-create')}
                >
                    Add New
                </Button>
                <Button
